...
const chooseApiAndSendResponse = (apiName, db, req, res, client, output) => {
  ...
  switch (apiName) {
    ...
    case "getRooms":
      ...
      break;
    case "addRoom":
      addroom(db, req, res, client, output, apiName);
    ...
    if (docs.length > 0) {
      ...
      newUser
        ...
        .then(() => {
          console.log("saved");
          output = newUser;
          console.log("IMporttant Output is ", output);
          sendResponseAndCloseConnection(client, output, res, apiName);
        })
        .catch(() => {
...
const addroom = (db, req, res, client, output, apiName) => {
  const { recieveid, senderid } = req.body;
  const newRoom = new Room({
    recieveid: recieveid,
    senderid: senderid,
  });
  Room.find({ recieveid: recieveid })
    .then(() => {
      output = "Already Created Room";
      sendResponseAndCloseConnection(client, output, res, output);
    })
    .catch(() => {
      newRoom.save().then(() => {
        output = "New Room Created";
        sendResponseAndCloseConnection(client, output, res, output);
      });
    });
};
...
const makeUpdateRoom = async (db, req, res, client, output, apiName) => {
  ...
  try {
    ...
    for (let i = 0; i < allMessages.length; i++) {
      ops.push(
        await bulk.find({ _id: ObjectId(allMessages[i].roomId) }).updateOne({
